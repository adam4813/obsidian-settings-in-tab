...
export default class SettingsInTab extends Plugin {
  async onload() {
    ...
    this.addRibbonIcon('settings', 'Open Settings in Tab', (evt: MouseEvent) => this.openSettingsInTab());
  }

  onunload() {
    // Clean up: remove our view if user disables plugin
    this.app.workspace.getLeavesOfType(SETTINGS_VIEW_TYPE).forEach((leaf) =>
      leaf.detach()
    );
  }

  async openSettingsInTab() {
    const existingLeaf = this.app.workspace.getLeavesOfType(SETTINGS_VIEW_TYPE)[0];
    if (existingLeaf) {
...
class AppSettingsView extends ItemView {
  ...
  async onOpen(): Promise<void> {
    ...
    this.corePluginTabs = (settingRegistry.pluginTabs as Array<any>).filter(tab => !tab.plugin.manifest).sort((a, b) => a.name.localeCompare(b.name));
    this.communityPluginTabs = (settingRegistry.pluginTabs as Array<any>).filter(tab => tab.plugin.manifest).sort((a, b) => a.name.localeCompare(b.name));
    ...
    this.navigationTabsContainer = mainContainer.createDiv('vertical-tab-header');
    this.contentContainer = mainContainer.createDiv('vertical-tab-content-container');
  ...
  private createNavigationTabItem(tab: any, groupContainer: HTMLElement): HTMLElement {
  ...
  private createNavigationTabItemGroup(tabs: any[], title: string): HTMLElement {
    const groupContainer = this.navigationTabsContainer.createEl('div', { cls: 'vertical-tab-header-group' });
    groupContainer.createEl('div', { text: title, cls: 'vertical-tab-header-group-title' });
  ...
  private renderNavigationTabs(): void {
    ...
    this.createNavigationTabItemGroup(this.communityPluginTabs, 'Community Plugins');
  }

  private showTab(tabId: string): void {
    // Update active sidebar item
    this.navigationTabsContainer.querySelectorAll('.vertical-tab-nav-item').forEach(item => {
    ...
    const selectedTab = this.settingTabs.find(tab => (tab.id || tab.name) === tabId);
    ...
    try {
      ...
      console.error(`Error displaying settings tab "${selectedTab.name}":`, error);
